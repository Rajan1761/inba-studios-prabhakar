...
// budget slider
const slider=document.querySelector('.budget-slider input');
const bval=document.querySelector('.budget-value b');
slider?.addEventListener('input',()=>{
  const v=+slider.value;bval.textContent='₹'+v.toLocaleString('en-IN');updateSummary();
});
// date/fields update
document.querySelectorAll('#book input,#book select').forEach(f=>f.addEventListener('change',updateSummary));
function updateSummary(){
  const name=document.querySelector('#book-name')?.value||'—';
  const pkg=document.querySelector('.chip.active')?.textContent||'Not selected';
  const date=document.querySelector('#book-date')?.value||'—';
  const budget=slider?slider.value:0;
  document.querySelector('.sum-name').textContent=name||'—';
  document.querySelector('.sum-pkg').textContent=pkg;
  document.querySelector('.sum-date').textContent=date;
  document.querySelector('.sum-budget').textContent='₹'+(+budget).toLocaleString('en-IN');
}
...
document.querySelector('#book-form')?.addEventListener('submit',e=>{
  e.preventDefault();
  const pkg=document.querySelector('.chip.active')?.textContent||'Shoot';
  const name=document.querySelector('#book-name')?.value||'Client';
  const phone=document.querySelector('#book-phone')?.value||'';
  const msg=`Hi Prabhakar, I'm ${name}. I'd like to book a ${pkg} shoot.`;
  window.open(`https://wa.me/919986431991?text=${encodeURIComponent(msg)}`,'_blank');
  showToast('Redirecting to WhatsApp booking…');
});
document.querySelector('.btn-whatsapp')?.addEventListener('click',()=>{
  window.open('https://wa.me/919986431991','_blank');
});
document.querySelector('.btn-email')?.addEventListener('click',()=>{
  window.location.href='mailto:inbastudios@gmail.com?subject=Booking%20Inquiry';
});
